...
| `themes/gruvbox-kvantum/` | EXCLUDED | License was not established from the local payload | `config/Kvantum/kvantum.kvconfig` now selects the system `kvantum-dark` theme. Live user theme files were not touched. |
| `config/btop/themes/gruvbox_dark_v2.theme` | EXCLUDED | License was not established from the local payload | btop now uses its verified built-in `Default` theme. |
