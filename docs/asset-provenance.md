...
| `themes/gruvbox-kvantum/` | REVIEW / EXCLUDED | The active local payload identifies Sourav Gope but contains no license notice; public source review is incomplete | The selector preserves the active `gruvbox-kvantum` name, but the theme files are not bundled or redistributed until licensing is confirmed. |
